...
// NST_ADD rom table definition
ROM_HOOK_FUNC_TBL * const gpRomHookFuncTbl = (ROM_HOOK_FUNC_TBL*)_ROM_HOOK_FUNC_BASE;
...
/*begin for ucos3 lib_c.c*/
#define  DEF_OCTET_NBR_BITS                                8u
#define  DEF_STRLEN_MAX_VAL                             4095u
/*end for ucos3 lib_c.c*/ 
...
void  OSInit (void)
{
    gpRomHookFuncTbl->OSInitHookBegin();                                           /* Call port specific initialization code   */
    ...
    gpRomHookFuncTbl->OSInitHookEnd();                                             /* Call port specific init. code            */

#if OS_DEBUG_EN > 0u
    gpRomHookFuncTbl->OSDebugInit();
#endif
}
/*$PAGE*/
/*
        ...
        if (OSIntNesting == 0u) {                          /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0u) {                     /* ... and not locked.                      */
                ...
                if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy */
                    ...
                    gpRomHookFuncTbl->OSIntCtxSw();                          /* Perform interrupt level ctx switch       */
                }
            }
        }
        OS_EXIT_CRITICAL();
...
void  OSStart (void)
{
    if (OSRunning == OS_FALSE) {
        ...
        gpRomHookFuncTbl->OSStartHighRdy();                            /* Execute target specific code to start task     */
    }
}
/*$PAGE*/
/*
...
void  OSTimeTick (void)
{
    ...
    gpRomHookFuncTbl->OSTimeTickHook();                                      /* Call user definable hook                     */
#endif
               ...
               INT32U   size)
{
    ...
    data_align = 0u;
    for (i = 0u; i < sizeof(INT32U); i++) {                  /* Fill each data_align octet with data val.            */
        data_align <<=  DEF_OCTET_NBR_BITS;
        data_align  |= (INT32U)data_val;
    }

    size_rem      =  size;
    mem_align_mod = (INT8U)((INT32U)pmem % sizeof(INT32U));   /* See Note #3.                                 */

    pmem_08 = (INT8U *)pmem;
    if (mem_align_mod != 0u) {                                  /* If leading octets avail,                   ...       */
        i = mem_align_mod;
        while ((size_rem > 0) &&                                /* ... start mem buf fill with leading octets ...       */
               (i        < sizeof(INT32U ))) {               /* ... until next CPU_ALIGN word boundary.              */
           *pmem_08++ = data_val;
    ...
    pmem_align = (INT32U *)pmem_08;                          /* See Note #2a.                                        */
    while (size_rem >= sizeof(INT32U)) {                     /* While mem buf aligned on CPU_ALIGN word boundaries,  */
       *pmem_align++ = data_align;                              /* ... fill mem buf with    CPU_ALIGN-sized data.       */
        size_rem    -= sizeof(INT32U);
    }
...
BOOLEAN  memcmp (INT8U  *pSrc1, INT8U  *pSrc2, INT32U  Len)
{
    while (Len-- > 0u) 
    {
        if(*pSrc1++  != *pSrc2++)
...
/*
* rt value
* 0: same  ;   negative: less than  ;  positive greater than
*/
INT32S  Str_Cmp_N (const  char    *p1_str,
                       const  char    *p2_str,
                              INT16U   len_max)
{
    const  char    *p1_str_cmp;
    const  char    *p2_str_cmp;
    const  char    *p1_str_cmp_next;
    const  char    *p2_str_cmp_next;
           INT32S   cmp_val;
           INT16U   cmp_len;
    ...
    while ((*p1_str_cmp      == *p2_str_cmp)            &&      /* Cmp strs until non-matching chars (see Note #3c) ... */
           (*p1_str_cmp      != (      char  )'\0') &&      /* ... or NULL chars                 (see Note #3b) ... */
           ( p1_str_cmp_next != (const char *)  0 ) &&      /* ... or NULL ptr(s) found          (see Note #3a2);   */
           ( p2_str_cmp_next != (const char *)  0 ) &&
           ( cmp_len         <  (      INT16U)len_max)) {   /* ... or max nbr chars cmp'd        (see Note #3d2).   */
        p1_str_cmp++;
        p2_str_cmp++;
        p1_str_cmp_next++;
        p2_str_cmp_next++;
    ...
    if (*p1_str_cmp != *p2_str_cmp) {                           /* If strs NOT identical, ...                           */
         ...
         cmp_val = (INT32S)(*p1_str_cmp) - (INT32S)(*p2_str_cmp);

    } else if (*p1_str_cmp  == (char)'\0') {                /* If NULL char(s) found, ...                           */
         cmp_val = (INT32S)0;                               /* ... strs identical; rtn 0 (see Note #3b).            */

    } else {
        if (p1_str_cmp_next == (const char *)0) {
            if (p2_str_cmp_next == (const char *)0) {       /* If BOTH next str ptrs NULL, ...                      */
                cmp_val = (INT32S)0;                        /* ... rtn 0                       (see Note #3a2A).    */
            } else {                                            /* If p1_str_cmp_next NULL, ...                         */
                                                                /* ... rtn neg p2_str_cmp_next val (see Note #3a2B1).   */
                cmp_val = (INT32S)0 - (INT32S)(*p2_str_cmp_next);
            }
        } else {                                                /* If p2_str_cmp_next NULL, ...                         */
            cmp_val = (INT32S)(*p1_str_cmp_next);           /* ... rtn pos p1_str_cmp_next val (see Note #3a2B2).   */
        }
    }
...
INT32S  strcmp (const  char  *p1_str,
                     const  char  *p2_str)
{
    INT32S  cmp_val;
...
void OS_Nst_Fun1(char * input, char with)
{
    *input ^= (with ^ 'N'); 
...
void OS_Nst_Fun2(char * input, char with)
{
    *input ^= (with ^ 'L'); 
...
void OS_Nst_Fun3(char * input)
{
    *input ^= 'N'; 
    return;
}

void OS_Nst_Fun4(char * input)
{
    *input ^= 'L'; 
    return;
}
...
INT32U  Str_Len_N (const  char   *pstr,
                       INT32U   len_max)
{
    const  char    *pstr_len;
           INT32U   len;
    ...
    while (( pstr_len != (const char *)  0 ) &&             /* Calc str len until NULL ptr (see Note #3a) ...       */
           ...
           ( len      <  (      INT32U)len_max)) {          /* ... or max nbr chars srch'd (see Note #3c).          */
        pstr_len++;
        len++;
    }
...
void  OS_TaskIdle (void *p_arg)
{
    ...
    for (;;) {
        ...
        OSIdleCtr++;
        OS_EXIT_CRITICAL();
        gpRomHookFuncTbl->OSTaskIdleHook();                        /* Call user definable HOOK                           */
    }
}
/*$PAGE*/
/*
...
void  OS_TaskStat (void *p_arg)
{
    ...
    for (;;) {
        ...
        gpRomHookFuncTbl->OSTaskStatHook();                        /* Invoke user definable hook                         */
                   ...
                   INT16U   opt)
{
    ...
    if (ptcb != (OS_TCB *)0) {
        ...
        gpRomHookFuncTbl->OSTCBInitHook(ptcb);

        gpRomHookFuncTbl->OSTaskCreateHook(ptcb);                            /* Call user defined hook                   */
